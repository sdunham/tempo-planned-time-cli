...
class Get extends Base {
  ...
  /**
   * Helper function to get the from and to dates for a given plan object 
   * returned by the Tempo API. In some cases (i.e. recurring plans), the 
   * startDate and endDate values aren't accurate so first we check for an 
   * object in the dates.values array that has what we need first.
   * @param {Object} plan A plan result object returned by the Tempo API
   * @returns {{from: string, to: string}} Object containing the 
   *                                       from and to dates
   */
  getPlanFromAndToDates(plan) {
    ...
    if (this.moment(to).isBefore(from)) {
      [to, from] = [from, to];
    }
  ...
  /**
   * Helper function to get the number of planned seconds for a given plan object 
   * returned by the Tempo API. In some cases (i.e. recurring plans), the 
   * secondsPerDay value isn't accurate so first we check for an object in the 
   * dates.values array that has what we need first.
   * @param {Object} plan A plan result object returned by the Tempo API
   * @returns {{int}} The number of planned seconds for the given plan
   */
  getPlanSeconds(plan) {
    const dateValues = plan.dates.values.find(value => 'timePlannedSeconds' in value);

    if (dateValues) {
      return dateValues.timePlannedSeconds;
    }

    return plan.secondsPerDay;
  }
  ...
  mapPlans(from, to, plans) {
    ...
    plans.forEach(plan => {
      const planDates = this.getPlanFromAndToDates(plan);
      const plannedSeconds = this.getPlanSeconds(plan);
      // Sanity check to make sure Tempo didn't return a plan that has 0 seconds planned 
      // (which can totally happen for some reason)
      if (plannedSeconds <= 0) {
        return;
      }
      const start = this.moment(planDates.from, 'YYYY-MM-DD');
      const end = this.moment(planDates.to, 'YYYY-MM-DD');
      const planRange = this.moment.range(start.startOf('day'), end.endOf('day'));
      ...
      const planData = {
        ...
        secondsPerDay: plannedSeconds,
        plannedTime: this.moment.duration(plannedSeconds, 'seconds').humanize()
      };
